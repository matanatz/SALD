...
class DeformNetwork(nn.Module):
    def __init__(self, conf, latent_size, auto_decoder):
        ...
        self.with_emb = conf.get_bool('decoder_implicit.with_emb')
        encoder_input_size = 6 if self.with_normals else 3

        logging.debug("AUTO DECODER : {0}".format(auto_decoder))
        ...
        self.prob_net =  nn.Sequential(nn.Linear(3 + latent_size, 128),nn.ReLU(),nn.Linear(128, self.K))
        # opt = torch.optim.Adam(self.prob_net.parameters())
        # loss_f = torch.nn.CrossEntropyLoss()
        # for i in range (1000):
        #     x = 4*torch.rand(64,3 + latent_size) - 2
        #     y = self.prob_net(x)
        #     loss = loss_f(y,(x[:,2] > 0).long())
        #     loss.backward()
        #     opt.step()
        #     opt.zero_grad()
        ...
        self.shape_interpolation = conf.get_int('shape_interpolation') # 0 mean , 1 line , 2
        self.detach_f = conf.get_bool('detach_f')
        self.lambda_i = conf.get_float('lambda_i')
        self.adapted_lr = conf.get_bool('adapted_lr')
        self.weighted_lsq = conf.get_bool('weighted_lsq')
        self.sigma_square = conf.get_float('sigma_square')
        self.noise_lsq = conf.get_float('noise_lsq')
        self.adaptive_with_sample = conf.get_bool('adaptive_with_sample')
        self.adaptive_epoch = conf.get_int('adaptive_epoch')

        logging.debug("""self.v_latent_size = {0},
                      self.predict_normals_on_surfce = {1}
                      self.sample_box = {2}
                      self.sample_method = {3}
                      self.rand_sample_factor = {4}
                      self.dist_sample_factor = {5}
                      self.v_sample_factor = {6}
                      self.v_start_unfirom = {7}
                      self.v_projection_steps = {8}
                      self.uniform_sample = {9}
                      self.dist_start_unfirom = {10}
                      self.proj_with_con = {11}
                      self.v_filter = {12}
                      self.t_samples = {13}
                      self.v_noise = {14}
                      self.con_dir = {15}
                      self.concate_proj = {16}
                      self.t_include_bndry = {17}
                      self.is_nearest = {18}
                      self.shape_interpolation = {19}
                      self.detach_f = {20}
                      self.lambda_i = {21}
                      self.K = {22}
                      self.adapted_lr = {23}
                      self.weighted_lsq = {24}
                      self.sigma_square = {25}
                      self.noise_lsq = {26}
                      self.adaptive_with_sample = {27}
                      self.adaptive_epoch = {28}""".format(self.v_latent_size,
                                                            ...
                                                            self.sigma_square,
                                                            self.noise_lsq,
                                                            self.adaptive_with_sample,
                                                            self.adaptive_epoch))



    def forward(self, manifold_points, manifold_normals, sample_nonmnfld, latent, latent_sigma_inputs, only_encoder_forward, only_decoder_forward,spider_head=None,is_additional_latent=False,z_func=None,epoch=-1,state=None,outside_sample=None,outside_dir=None,outside_latent=None):
        output = {}
        dirs = None
        ...
        if self.encoder is not None and not only_decoder_forward:
            ...
            dirs = (latent_draw - q_latent_mean)
            if self.dir_detach:
                dirs = dirs.detach()
        ...
        if only_decoder_forward:
            ...
            if (latent.shape[0] > 1 or outside_latent is not None) and self.encoder is None:
