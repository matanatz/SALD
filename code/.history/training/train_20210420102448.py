                ...
                if os.path.isfile(os.path.join(old_checkpnts_dir, 'ModelParameters', str(kwargs['checkpoint']) + ".pth")):
                    try:
                        if self.conf.get_bool('train.auto_decoder'):
                            ...
                            #data = torch.load(os.path.join(old_checkpnts_dir, "LatentCodes", str(kwargs['checkpoint']) + '_sigma.pth'))
                            #self.lat_vecs_sigma.load_state_dict(data['latent_codes'])
                        logging.info('after loading late vec')
                        saved_model_state = torch.load(os.path.join(old_checkpnts_dir, 'ModelParameters', str(kwargs['checkpoint']) + ".pth"))
                        # self.network.load_state_dict({**{k:v for k,v in saved_model_state['model_state_dict'].items() if not 'v_' in k},
                        #     **{k:v for k,v in self.network.state_dict().items() if 'v_' in k}})
                        self.network.load_state_dict({k:v for k,v in saved_model_state['model_state_dict'].items() if not 'temp' in k})
                        #self.network.load_state_dict({k:v for k,v in saved_model_state['model_state_dict'].items() if not 'prob' in k},strict=False)
