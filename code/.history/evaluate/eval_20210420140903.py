...
def optimize_latent(conf, latent, ds, itemindex, network,lat_vecs):
    ...
    loss_func = utils.get_class(conf.get_string('network.loss.loss_type'))(
        recon_loss_weight=1,grad_on_surface_weight=0,grad_loss_weight=0.1,z_weight=0.001)
    ...
    for e in range(num_iterations):
        ...
        outputs = network(pnts_mnfld, None, sample_nonmnfld[:,:,:3], latent, None, False, only_decoder_forward=False, spider_head=None)
...
def evaluate_with_load(gpu, conf, exps_folder_name, override, timestamp, checkpoint ,parallel, resolution, recon_only=False, plot_cmpr=True):
        ...
        if ((index == -1 or index == i )):
            ...
            if not recon_only:
                ...
                if (plot_cmpr  and i % 1 == 0):
                    ...
                    fig.layout.scene.update(dict(camera=dict(up=dict(x=0, y=1, z=0),xaxis=dict(range=[-1.5, 1.5], autorange=False),
                                                yaxis=dict(range=[-1.5, 1.5], autorange=False),
                                                zaxis=dict(range=[-1.5, 1.5], autorange=False),
                                                aspectratio=dict(x=1, y=1, z=1)))
                    fig.layout.scene2.update(dict(camera=dict(up=dict(x=0, y=1, z=0),xaxis=dict(range=[-1.5, 1.5], autorange=False),
                                                yaxis=dict(range=[-1.5, 1.5], autorange=False),
                                                zaxis=dict(range=[-1.5, 1.5], autorange=False),
                                                aspectratio=dict(x=1, y=1, z=1)))
                    if with_gt:
                        fig.layout.scene3.update(dict(camera=dict(up=dict(x=0, y=1, z=0),xaxis=dict(range=[-1.5, 1.5], autorange=False),
                                                    yaxis=dict(range=[-1.5, 1.5], autorange=False),
                                                    zaxis=dict(range=[-1.5, 1.5], autorange=False),
                                                    aspectratio=dict(x=1, y=1, z=1)))
