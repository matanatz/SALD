...
def evaluate(network,exps_folder_name, experiment_name, timestamp, ds, epoch, with_opt, resolution, with_normals, conf, index, chamfer_only, recon_only, lat_vecs, lat_vecs_sigma,with_ode=False,with_cuts=False,with_animation=True,visdomer=None,env='',step_log=None, split_filename = None, with_video = False,reuse_window=None,z_func=None,with_optimize_line=False,all_couples = 1,fix_coup=False,length=10,no_k=False,save_animation=True,retry=False,cancel_ffmpeg=False):
    ...
    for c in couples:
        ...
        arrows = []
        points = []
        layout = go.Layout(width=1200, height=1200, scene=dict(camera=camera ,xaxis=dict(range=[-3,3], autorange=False,showbackground=False,visible=False),
                                                                    yaxis=dict(range=[-3, 3], autorange=False,showbackground=False,visible=False),
                                                                    zaxis=dict(range=[-3, 3], autorange=False,showbackground=False,visible=False),
        ...
        if with_ode:
            ...
            data=[traces_ode[0]],
            layout=go.Layout(
                width=1200, height=1200, scene=dict(camera=camera,xaxis=dict(range=[-3, 3], autorange=False,visible=False),
                                                                    yaxis=dict(range=[-3, 3], autorange=False,visible=False),
                                                                    zaxis=dict(range=[-3, 3], autorange=False,visible=False),
                                                                    aspectratio=dict(x=1, y=1, z=1)),
                title="Start Title",
                updatemenus=[dict(
                    type="buttons",
        ...
        if len(traces) > 0:
            if with_animation and len(traces_anim) > 2:
                fig = go.Figure(
                    ...
                    layout=go.Layout(
                        width=1200, height=1200, scene=dict(camera=camera,xaxis=dict(range=[-3, 3], autorange=False,showbackground=False,visible=False),
                                                                            yaxis=dict(range=[-3, 3], autorange=False,showbackground=False,visible=False),
                                                                            zaxis=dict(range=[-3, 3], autorange=False,showbackground=False,visible=False),
                                                                            aspectratio=dict(x=1, y=1, z=1)),
                        title="Start Title",
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--resolution', default=64, type=int, help='Grid resolution')
