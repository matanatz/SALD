from  model.network_linear_prob_newloss_oldsolver_int import DeformNetwork
...
conf = """
{
    ...
    sigma_square = 0
    noise_lsq = 0
    shape_interpolation = 1
    adaptive_with_sample = False
    adaptive_epoch = 0
...
net = DeformNetwork(conf,256,True).cuda()


for s in [100,500,1000,2500,5000,7500,10000]:
    pnts = torch.rand([2,s,3]).cuda()
    latent = torch.rand([2,256]).cuda()
    latent = latent.requires_grad_(True)
    res = net(pnts,None,pnts,latent,None,False,False,spider_head=None,is_additional_latent=True,epoch=1)
    import time
    all = []
    for i in range(100):
        before = time.time()
        ...
        all.append(after)
        res['recon_surface'].sum().backward()
