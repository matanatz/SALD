...
if __name__ == '__main__':
    ...
    for cat in [x for x in sorted(os.listdir(os.path.join(datapath))) if not '.' in x and x in all_models['ShapeNet_processed'].keys()]:
            ...
            shapes = [x for x in sorted(os.listdir(source)) if not '.' in x]

            for shape in shapes:

                if (shapeindex == global_shape_index or shapeindex == -1):
                    print ("found!")
                    output_file = os.path.join(output,cat,shape)
                    print (output_file)
