...
if __name__ == '__main__':
    ...
    for cat in [x for x in sorted(os.listdir(os.path.join(datapath))) if not '.' in x and x in all_models['ShapeNet_processed'].keys() and x in ['04256520','03001627']]:
            ...
            utils.mkdir_ifnotexists(os.path.join(output,cat,'all_rot_y_v2'))
            ...
            for shape in shapes:
                ...
                if (shapeindex == global_shape_index or shapeindex == -1):
                    print ("found!")
                    output_file = os.path.join(output,cat,'all_rot_y_v2',shape)
                    print (output_file)
                    if (not args.skip or  not os.path.isfile(output_file + '_dist_triangle.npy')):
                        ...
                        mesh = as_mesh(mesh)
                        #rand_rot = trimesh.transformations.random_rotation_matrix()
                        theta = 2 * np.pi * np.random.rand()
                        rand_rot = np.array([[np.cos(theta),0,-np.sin(theta),0],
                                   [0,1,0,0],
                                   [np.sin(theta),0,np.cos(theta),0],
                                   [0,0,0,1]])
                        # save rot
                        np.save(output_file + '_rand_rot.npy',rand_rot)
