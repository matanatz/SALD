...
if __name__ == '__main__':
    ...
    for cat in [x for x in sorted(os.listdir(os.path.join(datapath))) if not '.' in x and x in all_models['ShapeNet_processed'].keys() and x in ['03001627','02691156']]:
            ...
            utils.mkdir_ifnotexists(os.path.join(output,cat,'all_rot_y'))
            ...
            for shape in shapes:
                ...
                if (shapeindex == global_shape_index or shapeindex == -1):
                    print ("found!")
                    output_file = os.path.join(output,cat,'all_rot_y',shape)
                    print (output_file)
