...
global_shape_index = np.array([len(x) for x in all_models['ShapeNet_processed'].values()]).sum()
...
template = '\n'.join(["#!/bin/csh",
                      ...
                      "cd /home/atzmonm/data/SALD/code/",
                      "echo before run",
            "python preprocess/preprocess_shapenet_rot.py --shapeindex {2}",
                      "echo after run"])
...
for run,i in enumerate(np.array_split(np.arange(1,length+1),length/100)):
    ...
    text_file.close()
    str_all.append("""qsub -q "all.q" -e /home/atzmonm/data/{0}/{1}_{2}.err -o /home/atzmonm/data/{0}/{1}_{2}.out ./{0}/arrayruns{1}.csh""".format('runs',run,"\$TASK_ID"))
