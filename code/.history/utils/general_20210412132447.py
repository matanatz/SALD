...
def configure_logging(debug,quiet,logfile):
    ...
    formatter = logging.Formatter("IM_FLOW - %(levelname)s - %(message)s")
...
def compute_trimesh_chamfer_v2(gt_points, gen_mesh, offset, scale,path,instance_name, num_mesh_samples=30000,one_side=False):
    """
    ...
    plt.plot_threed_scatter([[gen_points_sampled, 'pred'], [gt_points_np, 'gt']], path, instance_name, 0)
...
def compute_trimesh_chamfer(gt_points, gen_mesh, offset, scale, num_mesh_samples=30000):
    ...
    """
    d_thrsholds = np.linspace(0,0.04,20)
    gen_points_sampled = trimesh.sample.sample_surface(gen_mesh, num_mesh_samples)[0]
    ...
    # gt_points_np = gt_points.vertices
    gt_points_np = gt_points.vertices
    #plt.plot_threed_scatter([[gen_points_sampled, 'pred'], [gt_points_np, 'gt']], '.', 'b', 0)

    # one direction
    gen_points_kd_tree = KDTree(gen_points_sampled)
    one_distances, one_vertex_ids = gen_points_kd_tree.query(gt_points_np)
    gt_to_gen_coverage = (np.tile(np.expand_dims(one_distances,0),[d_thrsholds.shape[0],1]) < np.expand_dims(d_thrsholds,-1)).mean(-1)
    gt_to_gen_chamfer = np.mean(np.square(one_distances))

    # other direction
    gt_points_kd_tree = KDTree(gt_points_np)
    two_distances, two_vertex_ids = gt_points_kd_tree.query(gen_points_sampled)
    gen_to_gt_coverage = (np.tile(np.expand_dims(two_distances,0),[d_thrsholds.shape[0],1]) < np.expand_dims(d_thrsholds,-1)).mean(-1)
    gen_to_gt_chamfer = np.mean(np.square(two_distances))

    
    return {'gt_to_gen_chamfer':gt_to_gen_chamfer, 'gt_to_gen_coverage':gt_to_gen_coverage, 'gen_to_gt_chamfer': gen_to_gt_chamfer, 'gen_to_gt_coverage': gen_to_gt_coverage}
